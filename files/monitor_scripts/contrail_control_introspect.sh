...
def can_notify(utc_time, notify_minutes=10):
    t = datetime.now() - utc_time
            ...
            for peer_info in peer_infos:
                ...
                if e_identifier is not None:
                    ...
                    if e_deleted is not None:
                        if can_notify(get_time(last_event_at_text)):
                            status = STATUS.get("critical")
                            msg = "CRITICAL: " + "State:'{state}, {send_state}', LastEvent:{last_event}, LastEventAt={last_event_at}".\
                            format(state=state_text,
                                send_state=send_state_text, last_event=last_event_text,
                                last_event_at=get_time(last_event_at_text))
                            result.append(OUTPUT_FORMAT.format(status=status,
                                entity="XMPP:Peer:{name}".format(name=peer_name), message=msg))
        return result
        ...
        if not res.status_code in [200]:
            ...
            for r in rlist:
                ...
                if r.get("state") not in attrs["state"]["exp_value"] or r.get("send_state") not in attrs["send_state"]["exp_value"]:
                    status = STATUS.get("critical")
                    msg = "CRITICAL: " + "State:'{state}, {send_state}', LastEvent:'{last_event}', LastStateAt:{last_state_at}'.".\
                            format(state=r.get("state",""), send_state=r.get("send_state",""), last_event=r.get("last_event",""),
                                last_state_at=r.get("last_state_at",""))
                else:
                    status = STATUS.get("success")
                    msg = "OK: " + "State:'{state}, {send_state}', LastEvent:'{last_event}', LastStateAt:{last_state_at}'.".\
                            format(state=r.get("state",""), send_state=r.get("send_state",""), last_event=r.get("last_event",""),
                                last_state_at=r.get("last_state_at",""))
                result.append(OUTPUT_FORMAT.format(status=status,
                                                entity="{encoding}:peer:{name}".format(encoding=r.get("encoding"), name=peer_name), 
                                                message=msg))
                ...
                if e_ifmap_peer_server_conn_info is not None:
                   ...
                   if conn_status and host:
                       if "Up" in conn_status:
                          ...
                          monit_status = STATUS.get("critical")
